...
// seq plus a per-process stamp makes unit codes unique within a run AND
// across runs against a persistent database: a counter alone repeats next
// run, a nanosecond residue alone cycles within one.
var (
	seq      atomic.Int64
	runStamp = time.Now().UnixNano() % 1_000_000_000
)
...
func unit(t *testing.T, plat tenancy.PlatformPool, code string) string {
	code = fmt.Sprintf("%s-%d-%d", code, runStamp, seq.Add(1))
	t.Helper()
