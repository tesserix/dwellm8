...
| A11 | Add a property, "It's mine" | Property owned by the firm; `grant_id` empty on every later call | pass — property `KVH` minted under the firm, no grant. Re-run 2026-08-08 as `SMR`; the *second* self-owned property was refused outright until #361 |
...
| A13 | Onboard a tenant into a unit | Tenancy live on unit 101; rent, deposit and dates as entered | pass — first tenancy activates through the tax gate, so `active` not `pending_signature`. Re-run 2026-08-08: Arun Pillai active in Nair Gardens 301, ₹32,000 on day 5, ₹96,000 deposit |
| A14 | Today screen after the first tenancy | Rent roll and arrears real, not the demonstration figures | rent roll live; a tenancy that has not started is counted apart (#305) and named on the screen (#308); no sync queue is claimed (#309). The date and four tiles still demo — #291, #251. Re-run 2026-08-08: the figures are the open book's, but switching to it changed nothing on screen (#364); the tiles with no endpoint say so in as many words, so nothing is passed off as real |
...
| A19a | Onboard a second property you own yourself | The wizard offers your own books, and the property lands in them | pass — #361. The whole onboarding was refused with `E.164: ""`, losing property, units and tenancy with it |
| A19b | Read a refusal from the wizard | It reads as a refusal | pass — #362. Every message arrived in the success green, under a tick |
| A19c | Type a mobile the way it is read off a phone | Ten digits are enough, and text that is not a number is refused | pass — #360, #363. Before them the wizard sent the ten digits to a schema that wants E.164, losing the whole onboarding after the property had been created |
...
| F6 | Hostel or PG property — allocate beds by floor and room | Beds addressable individually; an allocated bed cannot be double-let | part pass — #367. Beds could not be put on the register from the app at all; with that fixed, G1-A and G1-B in one room read as 2-sharing, holding one drops the vacant count, and the states are the register's. Every room lands on floor 0 (#369), and a bed's rent cannot be corrected or the bed removed (#368). Double-letting is refused by the API (`beds_test.go`) but cannot be driven from the app: letting a bed means starting a tenancy, which is done from the property record |
| F7 | Bed allocation on a property that is neither hostel nor PG | Says so plainly rather than showing an empty grid | pass — with only a building under management the board reads "No hostel or PG yet — beds are allocated in a co-living building" and offers to onboard one, rather than an empty grid |
| F8 | Set viewing times | Slots stored per property; a past slot cannot be offered | pass — #370. Nothing could be run here until a flat could be advertised: viewing times hang off a listing and the app had no way to make one. With flat 102 on the market, Wednesdays and Saturdays at 10:00 published from a start date of 2026-08-01 — eight days in the past — and the dates ahead begin Wed 12 Aug. No past slot is offered. Times are held against the listing rather than the property, so two flats in one building can be shown at different hours |
| F9 | Two enquiries on one listing | Both appear as leads, newest first, each traceable to its listing | blocked — #371. Flat 102 is on the public search and a browsing token is issued, but `POST /v1/public/prospects/verify` answers "phone verification is not configured" and an unverified prospect cannot enquire. No enquiry can be made on any listing on the live environment, so no lead can reach the manager |
| F10 | Book a viewing into a slot | Slot marked taken; the lead's state advances | blocked — #371, same reason. The slots exist (F8) but only a verified prospect can book one |
