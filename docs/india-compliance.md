...
What Rentora must implement because of where it operates. Every rule below is
...
- Razorpay is the primary PA/PG. The adapter is written so a second provider can be added
  ...
  this org.
- Rentora is **not** an escrow or a custodian of client funds. Money settles owner-side
  through the regulated aggregator. Any design that has Rentora holding rent overnight in
...
- Aadhaar-based eSign uses the number transiently in the ESP flow; **Rentora never stores
...
- Every check requires an explicit, recorded consent artefact from the data principal
  naming the purpose.
- Screening outputs are advisory to the owner. Rentora does not publish a tenant score,
...
- The obligation stays with the deductor. Rentora's role is documented as facilitation,
...
| Regime | Relevance |
|---|---|
| **RERA** | Applies to listings and promoter/agent conduct. If Rentora lists or brokers, agent registration and advertising rules bind it. Claims in listings are a compliance surface. |
...
| **RBI PA/PG directions** | Bind the payment aggregator, and by extension how Rentora may hold or route funds. The "no escrow, no custody" rule above exists because of this. |
