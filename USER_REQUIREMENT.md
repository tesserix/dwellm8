...
|---|------|------------------------|----------------------|
| 1 | **GIP tenants** (#229) | **Created 2026-08-01**: `dwellm8-own-5945j`, `-ops-r8klu`, `-live-abowj`, `-find-xui6r`, `-pro-rk0lx`, `-admin-6izec`, password sign-up off; no sign-in providers configured yet, so the dev impersonation shim still serves dev | Enable phone OTP on all six and Google on own/ops/pro in the console (ADR-0027 §5), then remove the impersonation shim per #229's acceptance criteria |
...
| 11 | **docUrl signing key** (#212) | The full docUrl layer (signer, revocation, access log, fetch route) is built and tested, but `prod-dwellm8-docurl-key` does not exist in Secret Manager yet, so `esign.docUrlKeySecret` is `""` in the api chart and the route is unregistered | Create the SM secret (`openssl rand -base64 48` is fine — rotation only kills one signing window), set `esign.docUrlKeySecret: "prod-dwellm8-docurl-key"` in values. The document `Source` lands with the eSign flow (#62) |
